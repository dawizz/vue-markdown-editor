...
module.exports = function (md, options = {}) {
  ...
  function attrSet (token, name, value) {
    ...
    if (index < 0) { token.attrPush(attr) } else { token.attrs[index] = attr }
  }
  ...
  function makeCheckbox (token, TokenConstructor) {
    ...
    if (token.content.indexOf('[ ] ') === 0) { checkbox.content = renderCheckbox('todo') } else if (token.content.indexOf('[x] ') === 0 || token.content.indexOf('[X] ') === 0) { checkbox.content = renderCheckbox('completed') }
