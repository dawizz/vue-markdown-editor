...
export default function (md, { externalAttrs, openLinkIcon, openLinkIconClass }) {
  ...
  md.renderer.rules.link_open = (tokens, idx, options, env, self) => {
    ...
    if (hrefIndex >= 0) {
      ...
      if (isExternal) {
        ...
        if (/_blank/i.test(externalAttrs.target)) { hasOpenExternalLink = true }
      }
    }
  ...
  md.renderer.rules.link_close = (tokens, idx, options, env, self) => {
    if (hasOpenExternalLink) {
      ...
      if (openLinkIcon) {
        if (openLinkIconClass) { return `<i class="${openLinkIconClass}"></i>${self.renderToken(tokens, idx, options)}` }
