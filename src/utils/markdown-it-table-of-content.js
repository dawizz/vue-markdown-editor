...
export default function (md, o) {
  ...
  function toc (state, silent) {
    ...
    if (newline !== -1) { state.pos = newline } else { state.pos = state.pos + state.posMax + 1 }
  ...
  md.renderer.rules.toc_open = function (tokens, index) {
    ...
    if (options.containerHeaderHtml) { tocOpenHtml += options.containerHeaderHtml }
  ...
  md.renderer.rules.toc_close = function (tokens, index) {
    ...
    if (options.containerFooterHtml) { tocFooterHtml = options.containerFooterHtml }
  ...
  function renderChildsTokens (pos, tokens, slugs) {
    ...
    while (i < size) {
      ...
      if (
        ...
        !options.includeLevel.includes(level) ||
        heading.type !== 'inline'
      ) {
        i++
